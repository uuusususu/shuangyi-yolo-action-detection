...
![PyQtDarkTheme](https://img.shields.io/badge/Theme-PyQtDarkTheme_2.1.0-35D7FF)
...
| 结果反馈 | PASS / FAIL 独立声音提示，FAIL 证据图片保存 |
...
| --- | --- |
| 语言 | Python 3.11 |
| 桌面 UI | PySide6 + PyQtDarkTheme 2.1.0（MIT） |
| 视觉处理 | OpenCV |
| 推理引擎 | ONNX Runtime |
| 检测模型 | YOLO OBB |
| 工业相机 | MvSDK 封装 |
...
界面使用 MIT 许可的 PyQtDarkTheme 管理 PySide6 标准按钮、输入框、下拉框、数字框和复选框。项目保留深蓝背景、青色强调、步骤状态和 PASS/FAIL 业务色，主题库负责控件边界、箭头以及默认、悬停、按下、焦点、选中和禁用状态，避免把可点击控件误认为普通文字。
...
| yolo_action_detection/src/ui | PySide6 主界面、配置页、开源控件主题和科技风组件 |
...
| pass_sound_enabled | 是否启用 PASS 提示音，默认关闭 |
| fail_sound_enabled | 是否启用 FAIL 提示音，默认开启 |
| sound_feedback_enabled | 旧版兼容字段，保存时同步为 FAIL 提示音开关 |

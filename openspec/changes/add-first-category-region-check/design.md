...
- 同一异常签名进行去重/节流，避免同一 ID 同一数量错误每轮重复刷 NG。
...
- 事件层：声音、统计、证据保存、NG 记录按异常签名去重/节流。

建议异常签名为：

`(parent_id, step_key, observed_count, required_count)`

同一签名已上报过时，后续重试再次出现相同错误只保持红框和状态，不重复增加统计、播放声音或保存证据。若实际数量变化、异常步骤变化、父类重新进入新生命周期，或该 ID 曾经补齐后又发生新异常，则允许产生新 FAIL 事件。
...
- `event_queue`: 本帧新产生的 PASS/FAIL 事件，按 ID 和异常签名去重。
...
- [同一错误每轮重复刷 NG] -> 使用异常签名去重/节流，红框持续但外部事件不重复。
